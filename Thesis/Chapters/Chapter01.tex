...
\(\RR \langle x,x^* \rangle \) with an involution \(\phantom^*\) which sends
...
somehow analogous to the to real numbers while non-self-adjoint mattrices are
analogous to complex numbers with nonzero imaginary part {\color{red} how?}. In
light of this, if we view \(\RR \langle x \rangle \) as a the nc analogue of
polynomials with real variables while \(\RR \langle x,x^* \rangle \) is the nc
analogue of polynomials in complex variables! {\color{red} Techically the
  analogue is \emph{normal} matrices but honestly I don't wannt think about that
  right now.}
