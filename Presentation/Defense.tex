...
\begin{frame}
  ...
  \note{
    \begin{itemize}
      \item remark that these are the basic sets?
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Why should \(\nabla f\) be unique?}
  ...
  \note{
    \begin{itemize}
      \item {\color{blue} probably just give the idea of this proof---use the
            fact that tr( * ) is like an inner product and choose cooridinate matrices}
    \end{itemize}
  }
\end{frame}
...
\begin{frame}
  Go through the needed constructions -- direct sums of paths, analytic
  continuation of free functions, etc
\end{frame}
...
\begin{frame}
  ...
  \(\mathcal{F}(D)\).}
  \onslide<+->{
  For \( f \in \mathcal{F}(D)\), \(\nabla f \in \mathcal{T}(D)\)---so we have the
...
\begin{frame}
  ...
  \note{
    \begin{itemize}
      \item remark about global germs.
    \end{itemize}
  }
\end{frame}
...
\begin{frame}
  ...
  \onslide<+->{\textbf{Remark:} \(c^f\) only depends on the class of
    \(\nabla f\) in \(H^1_{\tr}(D)\)}
  \begin{align*}
...
\begin{frame}{Example: \(\pi_1^{\tr}(GL)\)}
  ...
  \vspace{\stretch{1}}
  \onslide<+->{ \(GL_1(\CC) = \CC \setminus \{0\} \), so
    \(\pi_1^{\tr}(D)_1 \simeq \ZZ \)}

  \vspace{\stretch{1}}
  \onslide<+->{Inclusion into \(\pi_1^{\tr}(D)_2\) picks up square roots. If
...
\begin{frame}
  \vspace{\stretch{1}}
  \onslide<+->{Similarly, inclusion into \(\pi_1^{\tr}(D)_3\) picks up cube
    roots:
  \[
